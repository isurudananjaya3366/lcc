...
def setup_test_tenant(django_db_setup, django_db_blocker):
    ...
    with django_db_blocker.unblock():
        ...
        try:
            ...
            # Tenant deletion may fail due to cross-schema FK cascades
            # (e.g. QuoteSettings FK → Tenant). Force-drop the schema directly.
            ...
            # Use raw SQL to avoid Django's CASCADE collector which would
            # try to query tenant-only tables (e.g. quotes_quotesettings)
            # that don't exist in the public schema.
            cursor.execute(
