...
// ================================================================
// Dropdown for choosing the heading font family.
// ================================================================
...
interface HeadingFontSelectorProps {
  value: string;
  onChange: (fontName: string, fontFamily: string) => void;
  className?: string;
}

const HEADING_CATEGORIES: FontCategory[] = ['sans-serif', 'serif', 'display'];

export function HeadingFontSelector({ value, onChange, className }: HeadingFontSelectorProps) {
  const [open, setOpen] = useState(false);
  const { status, load } = useFontLoader();

  const headingFonts = fontList.filter((f) => HEADING_CATEGORIES.includes(f.category));
